...
class SignUpViewController: UIViewController {
    ...
    @IBAction func onSignUpCButtonClicked(_ sender: Any) {
        ...
        guard let username = self.usernameTextEdit.text?.trimmingCharacters(in: .whitespacesAndNewlines) else {return}
        guard let email = self.emailTextEdit.text?.trimmingCharacters(in: .whitespacesAndNewlines) else {return}
        guard let password = self.passwordTextEdit.text?.trimmingCharacters(in: .whitespacesAndNewlines) else {return}
        guard let comfirmedPassword = self.comfirmPasswordTextEdit.text?.trimmingCharacters(in: .whitespacesAndNewlines) else {return}
